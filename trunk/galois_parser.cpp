...
void getChildren(Graph::GraphNode parent, Graph::GraphNode &child1, Graph::GraphNode &child2){
	...
	/*
	cout <<"parent "<< g.getData(parent).label_type;
	cout <<" child1 "<< g.getData(child1).label_type;
	cout <<" child2 "<< g.getData(child2).label_type<<'\n';
	*/
	//cout << " finished\n";
}
...
*/
	...
	while(!pq.empty()){
		...
		while(g.getData(pq.top()).level==level && !pq.empty()){
			//cout<<"Node inside level while 2 loop "<<level<<":"<<g.getData(pq.top()).label_type;
